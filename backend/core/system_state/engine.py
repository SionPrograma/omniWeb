...
class SystemStateEngine:
    ...
    async def get_state(self, force_refresh: bool = False, current_user: Optional[OmniUser] = None):
        ...
        if force_refresh or not self._state or (time.time() - self._last_update > current_ttl):
            await self.update_state(current_user=current_user)
        
        # Redact state for non-creators (V1.1 Surface Masking)
        if current_user:
            return self._redact_state_for_mode(self._state, current_user.mode)
        return self._state

    def _redact_state_for_mode(self, state: SystemState, mode: OmniMode) -> SystemState:
        """Deep copy and redact sensitive fields based on active mode."""
        from copy import deepcopy
        redacted = deepcopy(state)
        
        # Redact for ALL except CREATOR
        if mode != OmniMode.CREATOR:
            redacted.auditor_summary = None
            redacted.pending_fixes = 0
            redacted.cluster = None
            redacted.sync_status = None
            
        # Redact for TESTER
        if mode == OmniMode.TESTER:
             redacted.auditor_summary = {"status": "RESTRICTED"}
        
        # Redact for PUBLIC
        if mode == OmniMode.PUBLIC:
            redacted.chips = []
            redacted.active_mission = None
            redacted.archived_missions = []
            redacted.completed_missions = []
            redacted.proposals = []
            redacted.mission_events = []
            redacted.portfolio_pulse = []
            redacted.last_handoff = None
            redacted.flow_data = {}
            redacted.memory_usage = {}
            
        return redacted

    async def update_state(self, current_user: Optional[OmniUser] = None):
        """Aggregates status from all subsystems into a unified state object."""
        # MISSION 127: Set clear identity for headless system-level updates
        # This allows DB access under narrow system permission list.
        effective_slug = "system:monitor"
        
        user_id = current_user.id if current_user else None
        with set_chip_context(effective_slug, user_id=user_id):
            ...
            try:
                ...
                self._state = SystemState(
                    ...
                    system_mode=mode,
                    mode=current_user.mode if current_user else OmniMode.PUBLIC,
